...
// function extractToken(req) {
//   if (req.cookies?.token) return req.cookies.token;              // HttpOnly cookie
//   const h = req.headers.authorization || req.headers.Authorization;
//   if (h && h.startsWith("Bearer ")) return h.split(" ")[1];      // Bearer header
//   return null;
// }
...
//   next();
// };
// module.exports = { userAuth, isAdmin, verifyAdmin:[userAuth, isAdmin] };
...
const userAuth = async (req, res, next) => {
  try {
    ...
    // 2) না থাকলে Authorization: Bearer <token> থেকে নাও
